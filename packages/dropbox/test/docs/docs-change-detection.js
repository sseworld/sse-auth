...
describe("Docs change detection", function () {
  ...
  describe("", () => {
    ...
    it("contains equal content", function (done) {
      try {
        const currentDocsDescription = normalizeString(
          JSON.stringify(
            decompress(
              JSON.parse(fs.readFileSync(API_DESC_FILE_PATH).toString()),
            ),
            null,
            "\t",
          ),
        ...
        generate((err, retrievedDocsDescription) => {
          ...
          retrievedDocsDescription = normalizeString(
            JSON.stringify(retrievedDocsDescription, null, "\t"),
          );
          expect(md5(retrievedDocsDescription)).to.be(
            md5(currentDocsDescription),
          );
          done();
        });
      } catch (err) {
