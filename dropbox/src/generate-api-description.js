...
async function generateApiDescription2(cb) {
  ...
  if (cb) {
    ...
    fs.writeFileSync(
      "./src/api.json",
      JSON.stringify(compressor.compress(apiExamples)),
    );
  }
}
  ...
  var namespaces = _.map(resp.children(), function (sectionChild) {
    ...
    var methodDescWrapElems = _.map(sectionChild.children, function (child, i) {
      if (
        child.attribs &&
        child.attribs.class &&
        utils.hasClass("toc-el", child.attribs.class)
      ) {
        return child;
      }
    });
    ...
    return _.map(el.find(".field"), function (item) {
      ...
      if (!!nestedWrap.length) {
        ...
        return {
          ...
          desc: desc,
          parameters: _.flatten(
            _.map(nestedWrap, function (item) {
...
function createApiObject(fullApiObject) {
  Object.keys(fullApiObject).forEach((name) => {
    ...
    apiDescription.parameters.available =
      apiDescription.parameters.list.length > 0;
...
function parseApiDescription(apiDescription) {
  ...
  _.each(apiDescription, function (namespace) {
    ...
    _.each(namespace.methods, function (method) {
      ...
      var requiresAuthHeader =
        methodExample === null
          ? true
          : utils.contains(methodExample, HEADER_AUTH);
      var requiresReadableStream =
        methodExample === null
          ? false
          : utils.contains(methodExample, HEADER_CT_OCTET_STREAM);
