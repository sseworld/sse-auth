...
const updateRequestOptsFnList = [
  ...
  (requestOpts, resourceDescription, userOpts, config) => {
    ...
    if (resourceCategory === RPC_RESOURCE_CATEGORY) {
      ...
      requestOpts.headers[DB_HEADER_API_ARGS] = isObject(userParameters)
        ? http_header_safe_json(userParameters)
        : "";
    }
  },
];
...
function generateResourcesHandlingFunctions(resourcesDescriptionList, config) {
  ...
  Object.keys(resourcesDescriptionList).forEach((resourceName) => {
    ...
    resourcesHandlingFunctions[resourceName] = function (userOpts, userCb) {
      ...
      updateRequestOptsFnList.forEach((updateRequestOptsFn) =>
        updateRequestOptsFn(requestOpts, resourceDescription, userOpts, config),
      );
        ...
        return (err, response, body) => {
          ...
          const handleResponseByContentType = {
            ...
            "application/octet-stream": () => {
              const dropboxApiResultContent =
                response.headers[DB_API_RESULT_HEADER_NAME];
              return (
                dropboxApiResultContent &&
                userCb(null, JSON.parse(dropboxApiResultContent), response)
              );
            },
          ...
          const responseHandlerFn =
            handleResponseByContentType[responseContentType];
...
module.exports = {
  authenticate: function (config) {
    const resourceHandlingFunctions = generateResourcesHandlingFunctions(
