...
var parsedApiDescription = JSON.parse(
  fs.readFileSync(path.join(__dirname, "../dist/api.json")),
);
const options = {};
...
const mdContent = apiNameList.map((apiName) => {
  ...
  parsedApiDescription[apiName].request = parsedApiDescription[apiName]
    .request || { example: [{}] };

  const example = prepareExampleByApiDescription(
    parsedApiDescription[apiName],
    apiName,
  );
  return `
...
parameters: <%- JSON.stringify(request.example[0].value, null, '') %> <% } %>}, (err, result, response) => {
  ...
  const code = beautify(template(apiDescription), { indent_size: 4 }).replace(
    /\"/g,
    "'",
  );
